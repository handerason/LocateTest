...
object useTraceModel {
  def main(args: Array[String]): Unit = {
    ...
    val tmpRDD: RDD[Trace] = traceRDD.map(x => {
      val G4: mutable.HashMap[String, String] = G4Broad.value
      x.wheres.foreach(y => {
        val district: String = G4(y.code)
        y.code = district
      })
      x
    })

    val combineTrace: RDD[Trace] = tmpRDD.map(x => {
      // 上边识别完成之后进行合并，将相同区县的数据进行合并
...
//      }
    ...
    val resultRDD: RDD[(String, String, String, String, String,String)] = combineTrace.filter(_.wheres.length == 1).flatMap(x => {
      val result = new ListBuffer[(String, String, String, String, String,String)]

      x.wheres.foreach(y => {
        result.append((x.imsi.split("\t")(0), x.imsi.split("\t")(1), y.code, y.sTime.toString, y.eTime.toString, x.imsi.split("\t")(3)))
      })
      result
    }).filter(_._2.isEmpty)
    ...
    val result: RDD[Row] = resultRDD.map(x => {
      Row(x._1, x._2, x._3, x._4, x._5, x._6)
    })

    result.saveAsTextFile(outputPath)

    sc.stop()
    println("success!————用户轨迹模型" + date)
  }
