 ...
 */
class GTrace(val traceType: LocType = LocType.GRID) extends Comparable[GTrace] with Serializable {
  var imsi: String = _ //用户标识
  var date: String = _ //UTF型日期
  var ecis = new mutable.HashMap[String,ListBuffer[Long]]
  ...
  def getAllEciList(records: List[Record]):Unit = {
    for (elem <- records) {
      val code: String = elem.getCode(LocType.GRID)
      if (ecis.exists(x => code.equals(x._1))){
        val eciList: ListBuffer[Long] = ecis(code)
        eciList.append(elem.eci)
        ecis.remove(code)
        ecis.put(code,eciList)
      }else{
        val eciList = new ListBuffer[Long]
        eciList.append(elem.eci)
        ecis.put(code,eciList)
      }
    }
   ...
   * @return 子Trace
   */
  def getSubTrace(stime: Long, etime: Long): GTrace = {
    val subTrace: GTrace = new GTrace(imsi, traceType) //保存结果
    for (where <- wheres) {
      breakable {
  ...
  def getSubTraces(sCode: String, eCode: String): ListBuffer[GTrace] = {
    val subTraces = new ListBuffer[GTrace] //保存结果
    var subTrace: GTrace = null //初始化为null
    var sWhere: Where = null //标记sCode对应的where
    for (where <- this.wheres) {
      //如果where的编码==sCode则认为找到swhere
      if (where.code.equals(sCode)) {
        //生成子trace,并装载where
        subTrace = new GTrace(imsi, traceType)
   ...
   * @return 居住地到工作地的轨迹
   */
  def getHome2WorkTraces(user: User, locType: LocType): List[GTrace] = {
    var traces: ListBuffer[GTrace] = new ListBuffer[GTrace]
   ...
   * @return 工作地到居住地的轨迹
   */
  def getWork2HomeTraces(user: User, locType: LocType): List[GTrace] = {
    var traces: ListBuffer[GTrace] = new ListBuffer[GTrace]
    if (locType.equals(LocType.AOI)) {
  ...
  def get1DayOd(user: User, minStayTime: Int = 60, locType: LocType, work: Work): Option[Od] = {
    ...
    val subTrace: GTrace = getSubTrace(sTime, work.eTime(date))
   ...
   * @return 平均通勤时长
   */
  def avgHome2WorkTime(traces: List[GTrace], user: User, locType: LocType): Int = {
   ...
   * @return 平均通勤时长
   */
  def avgWork2HomeTime(traces: List[GTrace], user: User, locType: LocType): Int = {
    val times = new ListBuffer[Int]()
    for (trace <- traces) {
  ...
  override def compareTo(o: GTrace): Int = {
    var i: Int = 0
