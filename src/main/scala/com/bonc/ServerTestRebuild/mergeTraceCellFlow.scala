...
/**
 * 基于融合定位数据生成用户的基站进入进出模型
 *
 * @author opslos
 * @date 2021/4/26 16:30:09
 */
object mergeTraceCellFlow {
  def main(args: Array[String]): Unit = {
    ...
    val recordRDD: RDD[(String, Record)] = inputRDD.map(x => {
    ...
    val resultRDD: RDD[(String, String, String, String, String, String, String, String, String, String, String, String, String, String)] = traceRDD.flatMap(x => {
      ...
      x.wheres.foreach(y => {
        ...
        var city = ""
        if (y.rat == 2) {
          val dc: String = G2B.getOrElse(y.code, "")
          if (dc.nonEmpty) {
            city = dc.split("_")(0)
            district = dc.split("_")(1)
          }
        } else {
          val dc: String = G4B.getOrElse(y.code, "")
          if (dc.nonEmpty) {
            city = dc.split("_")(0)
            district = dc.split("_")(1)
          }
