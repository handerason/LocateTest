...
/**
 * 直接用用户的融合定位轨迹根据码表用户县级别的OD
 * 区县和市区的可以用一套，不同的地方在于工参的处理
 * 市区的工参是对应市区信息，区县的工参是对应区县的信息
 *
 * @author opslos
 * @date 2021/5/6 10:45:14
 */
object countyBetweenOD {
      ...
      if (!gc.contains("-")) {
        ...
        if (gc.split("\\|").length >= 5) {
            if (gc.split("\\|")(2).contains("_")) {
              G2List.put(gc.split("\\|")(2), gc.split("\\|")(6) + "_" + gc.split("\\|")(5) + "_" + gc.split("\\|")(0))
            } else {
              G4List.put(gc.split("\\|")(2), gc.split("\\|")(6) + "_" + gc.split("\\|")(5) + "_" + gc.split("\\|")(0))
            }
        }
      }
      gc = gcReader.readLine()
